...
import type { FlightLlmReviewCandidate } from "@workspace/domain";
...
function getCandidateStatusLabel(candidate: FlightLlmReviewCandidate) {
  switch (candidate.status) {
    case "failed": {
      return "Failed heuristic pass";
    }
    case "no_match": {
...
interface FlightLlmReviewSheetProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  fromDate: string | null;
  candidates: FlightLlmReviewCandidate[];
  isLoading: boolean;
...
export function FlightLlmReviewSheet({
  open,
  onOpenChange,
  fromDate,
  candidates,
      ...
      >
        <SheetHeader className="border-b border-border/60 pb-4 text-left">
          <SheetTitle className="flex items-center gap-2">
            ...
            Review Emails For LLM Fallback
          </SheetTitle>
          <SheetDescription>
            {fromDate
              ? `Only unmatched flight emails received on or after ${fromDate} are shown here. JSON-LD and heuristic extraction have already run.`
              : "Choose which unmatched flight emails should use the LLM fallback."}
          </SheetDescription>
        </SheetHeader>

        <div className="flex items-center justify-between border-b border-border/60 py-3 text-sm">
          <div className="text-muted-foreground">
            {selectedIds.length} selected of {candidates.length}
          </div>
            ...
            {!isLoading && !errorMessage && candidates.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border/70 px-4 py-10 text-center">
                <p className="text-base font-medium text-foreground">
                  No LLM candidates right now
                </p>
                <p className="mt-2 text-sm text-muted-foreground">
                  Everything in this window either matched without the LLM or
