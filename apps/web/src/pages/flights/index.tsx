...
import type { FlightActivity, UpdateFlightActivityInput } from '@workspace/domain'
...
export default function FlightsPage() {
  ...
  const [methodFilter, setMethodFilter] = useState<
    (typeof EXTRACTION_METHOD_OPTIONS)[number]['value']
  >('all')
  const [editorOpen, setEditorOpen] = useState(false)
  const [selectedActivity, setSelectedActivity] = useState<FlightActivity | null>(
    null,
  )
  ...
  const disconnectMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: flightKeys.gmailStatus() })
    },
  })

  const { startSync, startReprocess, job, isSyncing, progress }
    = useFlightSyncJob()
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {statusQuery.data?.connected ? 'Reconnect Gmail' : 'Connect Gmail'}
              </Button>
              <Button
                variant="outline"
          ...
          <Card className="border-border/60 bg-card/90">
            ...
            <CardContent className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {statusQuery.data?.email ?? 'Connect Gmail to sync booking emails.'}
              </p>
            </CardContent>
          </Card>
              ...
              {stats.nextDeparture
                ? (
                    <>
                      ...
                      <p className="mt-2 text-sm text-muted-foreground">
                        {stats.nextDeparture.flightNumber}
                        {' '}
                        on
                        {' '}
        ...
        <Card className="border-border/60">
          <CardHeader className="gap-4">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <CardTitle className="text-base">Captured Flights</CardTitle>
                <p className="mt-1 text-sm text-muted-foreground">
                  Search and correct extracted itineraries. Click a row action
                  to edit with source-email context.
                </p>
              </div>
            ...
            {activitiesQuery.isError
              ? (
                  <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-6 text-sm text-destructive">
                    We couldn’t load flights right now. Try again after the current
                    sync completes.
                  </div>
                )
              : null}
