 ...
 * Anemic design - works with DTOs and services handle all business logic
...
export class AuthService {
  ...
  async login(email: string, password: string, deviceContext?: DeviceContext) {
    ...
    // 3. Update authentication identity (update updatedAt)
  ...
  private async generateTokens(
    ...
    // Create session DTO (create first to get sessionId)
    const sessionId = randomUUID()
    const now = new Date()
    const session = new AuthSessionDto({
      id: sessionId,
      userId,
      token: refreshToken,
      expiresAt,
